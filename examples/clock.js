...
// ease of establishing a trivial isomorphism between syntax trees and graphical elements.
    ...
    overlay_prototype = capture [bind(name, f)    = this -se- (it['_#{name}_listeners'] = it['_#{name}_listeners'] || []).push(f),
                                 ...
                                 children(xs = arguments) = +xs *~![x instanceof this.constructor ? [x] : x] *!this.push -seq -re- this,
...
// Clock application | Spencer Tipping
// Licensed under the terms of the MIT source code license
...
caterwaul.js_all()(function ($) {
  ...
  -where [render_frame()            = console.log('%s', clock_overlay().position([0, 1]).render_root()) -se- query_size(),
          ...
          container(xs = arguments) = new text_overlay() -re- it /-it.children.apply/ xs,
          ...
          clock_ring                = ring(1),
          second_hand_ring          = ring(0.9),
          minute_hand_ring          = ring(0.7),
          hour_hand_ring            = ring(0.4),
          weekday_ring              = ring(0.8),
          day_of_month_ring         = ring(0.7),
          month_ring                = ring(0.6),

          center                    = [0, 0],

          spaces(n)                 = n[n] *[' '] -seq -re- it.join(''),

          clear()                   = n[height + 1] *[new text_overlay(spaces(width + 1)).position([0, x])] /seq /!container,
          ...
          hands()                   = hour_hand() /minute_hand() /second_hand() /!container
                              -where [hand(c, p, mark) = line(c, c /-v2plus/ p) /~push/ new text_overlay(mark).fg(yellow).position(c /-v2plus/ p),
                                      hour_hand()      = hand(center, hour_hand_ring(new Date().getHours()     / 12 + new Date().getMinutes() / 720), '.'),
                                      minute_hand()    = hand(center, minute_hand_ring(new Date().getMinutes() / 60 + new Date().getSeconds() / 3600), '.'),
                                      second_hand()    = hand(center, second_hand_ring(new Date().getSeconds() / 60 + +new Date() % 1000 / 60000), '.')],
