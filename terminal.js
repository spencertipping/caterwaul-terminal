...
// Everything else is either lower ASCII or a Unicode character, which is considered to be regular input.
                ...
                -where [parse_input(overlay, s)  = overlay -se- s.split('\033[') *![overlay /-normal_keystrokes/ parse_escape(overlay, x)] -seq,
                        ...
                        mouse_event(o, s)        = s.charCodeAt(0) & 0x8 ? o /~mouseup/ mouse_event_for(s, 'mouseup') : o /~mousedown/ mouse_event_for(s, 'mousedown'),
                        mouse_event_for(s, name) = $.terminal.event[name](s.charCodeAt(0) & 0x7, [s.charCodeAt(1) - 32, s.charCodeAt(2) - 32], s.charCodeAt(0) >> 4),

                        arrow_event(o, s)        = o /~control/ $.terminal.event.arrow(s),
                        control_event(o, s)      = o /~control/ $.terminal.event.control(s),
                        escaped_key(o, s)        = o /~control/ $.terminal.event.escaped(s),
...
// All events come with some data that may be helpful:
...
//   modifiers()   a bitmask of modifier keys such as control, shift, and alt -- available only for mouse events. shift = bit 0, meta = bit 1, control = bit 2
    ...
    event = given [data, target] [this instanceof $.terminal.event ? this -se [it._data = data, it._target = target || null] : new $.terminal.event(data, target)]
            -se- it.prototype           /capture [stop() = this._stopped = this,  target() = this._target]
  ...
  where [pluralize(f)(xs)  = xs *[[x, f(x)]] -object -seq,
         ...
         event(name)(x)    = x instanceof Function ? this.bind(name, x) : this.trigger(name, x),   events    = pluralize(event)]})(caterwaul);
