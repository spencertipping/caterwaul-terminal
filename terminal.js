...
//   rows and columns available. Any irrelevant data is sent to stdin_cc.
  ...
  // Note: This is a highly questionable abstraction. I'm going to try to think of a better way to do the IO encapsulation.

    detect_size(stdout_cc, stdout_cc('\033[1048576;1048576H'))(stdin_data, size_cc, stdin_cc) = /^(.*)\033\[(\d+);(\d+)R(.*)$/.exec(stdin_data)
                                                                                                -re [it ? stdin_cc('#{it[1]}#{it[4]}') -re- size_cc([+it[2], +it[3]]) :
                                                                                                          stdin_cc(stdin_data)],
